...
export const add = new Command()
  ...
  .option('-a, --all', 'Add all hooks', false)
  .option('-p, --path <path>', 'The path to the hook file')
  .action(async (hooks, opts) => {
    try {
      const options = addOptionsSchema.parse(hooks, ...opts);
      ...
      const spinner = ora({ text: `Installing hooks...` });
      // spinner.start();
      ...
      for (const item of payload) {
        ...
        const existingHooks = item.files.filter((file: File) =>
          existsSync(path.resolve(targetDir, file.name))
        );

        if (existingHooks.length && !options.overwrite) {
